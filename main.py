...
os.environ.setdefault("ULTRAVOX_API_KEY", "PRAv5rQr.O2580pzU9yHuWiA0vx3Mgs4H5f6WraM6")
...
class WebsocketVoiceSession(pyee.asyncio.AsyncIOEventEmitter):
    ...
    def __init__(self, join_url: str, menu_name: str = "small"):
        super().__init__()
        ...
        self._menu = get_menu(menu_name)
        self._cart_normalizer = CartNormalizer(self._menu)
        self._cart = Cart()
        self._user_transcripts = []  # Store user transcripts for cart parsing
    ...
    async def start(self):
        ...
        try:
            ...
            logging.info("Receive task created successfully")
    ...
    async def _handle_data_message(self, msg: dict[str, Any]):
        match msg["type"]:
            ...
            case "transcript":
                ...
                if msg["role"] == "user":
                    # Store user transcripts for cart parsing
                    if msg.get("text", None):
                        self._user_transcripts.append(msg["text"])
                        # Emit user input for display
                        self.emit("user_input", msg["text"], msg["final"])
                        # Parse the cart from user transcript
                        self._update_cart_from_user_input(msg["text"])
                    elif msg.get("delta", None):
                        # For incremental transcripts
                        self._pending_user_input = getattr(self, '_pending_user_input', '') + msg["delta"]
                        self.emit("user_input", self._pending_user_input, msg["final"])
                        if msg["final"]:
    ...
    def _update_cart_from_user_input(self, user_input: str):
        """Parse user input and update the cart."""
        try:
            # Parse the order
            parsed_cart = self._cart_normalizer.parse_order(user_input)
            
            # Update the cart
            self._cart = parsed_cart
            
            # Emit cart update event
            self.emit("cart_update", self._cart)
        except Exception as e:
            logging.warning(f"Error parsing cart from user input: {e}")
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        try:
            async with session.post(target, headers=headers, json=body) as response:
                ...
                logging.info(f"Join URL: {join_url}")
                return join_url
        except Exception as e:
...
async def main():
    ...
    menu_name = args.menu if hasattr(args, 'menu') else "small"
    client = WebsocketVoiceSession(join_url, menu_name)
    done = asyncio.Event()

    # Track user input for display
    user_input_buffer = ""
    ...
    @client.on("user_input")
    async def on_user_input(text, final):
        """Display user input as they speak."""
        nonlocal user_input_buffer
        if final:
            user_input_buffer = ""
            print(f"You said: {text.strip()}")
        else:
            user_input_buffer = text
            print(f"You said: {text.strip()}", end="\r")

    @client.on("cart_update")
    async def on_cart_update(cart: Cart):
        ...
        for idx, item in enumerate(cart.items, 1):
            ...
            modifiers_str = f" [{', '.join(item.modifiers)}]" if item.modifiers else ""
            print(f"{idx}. {item.quantity}x {item.name}{size_str}{modifiers_str} - ${item.price:.2f} each")
        print("-"*60)
    ...
    # Start the client first, then wait
    await client.start()
    print("Connected! You can now speak your order...")
...
CARAMEL MOCHA LATTE $3.49""",
    ...
    parser.add_argument(
        ...
        choices=["small", "large"],
        default="small",
        help="Menu size to use for cart normalization (default: small)",
    )
